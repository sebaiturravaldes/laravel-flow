...
class FlowBuilder
{
	...
	*/
	public function build_response($result){
		$flow_comercio  = config('flow.comercio');
	...
	*/
	public function flow_log($message, $type) {
		$flow_logPath = config('flow.logPath');
		$file         = fopen($flow_logPath . "/flowLog_" . date("Y-m-d") .".txt" , "a+");
		$data         = "[".date("Y-m-d H:i:s.u")." ".$_SERVER['REMOTE_ADDR']." ";
		
		if (array_key_exists('HTTP_X_FORWARDED_FOR', $_SERVER)) {
			$data .= $_SERVER['HTTP_X_FORWARDED_FOR'];
		}

		$data .=" - $type ] ".$message . PHP_EOL;
		fwrite($file, $data);
		fclose($file);		   
	}
	...
	private function flow_pack() {
		...
		$url_exito        = urlencode(config('flow.url_exito'));
		$url_fracaso      = urlencode(config('flow.url_fracaso'));
		$url_confirmacion = urlencode(config('flow.url_confirmacion'));
		$url_retorno      = urlencode(config('flow.url_retorno'));
